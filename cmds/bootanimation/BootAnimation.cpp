...
/* Vanzo:fenghaitao on: Fri, 15 May 2015 14:33:01 +0800
 * add power on sound prop
 */
        char bootVolume[PROPERTY_VALUE_MAX];
        property_get("persist.sys.boot_ringtone", bootVolume, "1");
        int nBootVolume =1;
        XLOGD("BootAnimation   bootVolume this is in BootAnimation ... %s",bootVolume);
        nBootVolume= atoi(bootVolume);
        if(nBootVolume == 0){
            XLOGD("initAudioPath:Profile off.... DON'T PLAY AUDIO!");
            return NULL;
        }
