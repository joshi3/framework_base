...
public abstract class BaseSignalClusterExt implements ISignalClusterExt {
    ...
    private BasePhoneStateExt getOrInflatePhoneState(int subId,
        ...
        if (DEBUG) {
            Log.d(TAG, "getOrInflatePhoneState(), state = " + state);
        }
        return state;
    }
    ...
    public void apply() {
        ...
        if (DEBUG) {
            Log.d(TAG, "apply(), start"
                    ...
                    + ", mWideTypeIconStartPadding = " + mWideTypeIconStartPadding
                    + ", mSecondaryTelephonyPadding = " + mSecondaryTelephonyPadding
                    + ", mMobileSignalGroup.ChildCount = " + mMobileSignalGroup.getChildCount());
        }
    ...
    protected final int getSlotId(int subId) {
        ...
        if (DEBUG) {
            Log.d(TAG, "getSlotId(), subId = " + subId + ", slotId = " + slotId);
        }
    ...
    protected final boolean isSimInsertedBySlot(int slotId) {
        ...
        if (SubscriptionManager.isValidSlotId(slotId)) {
            ...
            isSimInserted = SubscriptionManager.isValidSubscriptionId(subId);
        }
        return isSimInserted;
    }

    protected static final boolean isSimInsertedBySub(int subId) {
    ...
    protected class BasePhoneStateExt extends PhoneStateExt {
        ...
        public boolean apply() {
            Log.d(TAG, "apply(), mSlotId = " + mSlotId + ", mSubId = " + mSubId
                    + ", PhoneState = " + this.toString());
        ...
        protected void toString(final StringBuilder builder) {
            ...
            builder.append(',')
                    ...
                    .append("mIsSimAvailable=").append(mIsSimAvailable).append(',')
                    .append("mIsSimOffline=").append(mIsSimOffline).append(',')
                    .append("mRoaming=").append(mRoaming).append(',')
                    .append("mDataConnectioned=").append(mDataConnectioned).append(',')
                    .append("mSignalStrengthLevel=").append(mSignalStrengthLevel).append(',')
                    .append("mNetworkType=").append(mNetworkType).append(',')
                    .append("mDataType=").append(mDataType).append(',')
                    .append("mDataActivity=").append(mDataActivity);
        }
        ...
        protected boolean isSignalStrengthNullIcon() {
            final boolean isNullIcon = mMobileStrengthIcon == mDefaultSignalNullIconId
                    .getIconId();
            if (DEBUG) {
                Log.d(TAG, "isSignalStrengthNullIcon() = " + isNullIcon);
            }
            return isNullIcon;
        }
