...
/* Vanzo:zhangjingzhi on: Tue, 08 Apr 2014 11:47:58 +0800
 * for customized emergency number
 */
        String customNumberstring= SystemProperties.get("persist.sys.emergencynumber","no_customized_number");

        if((customNumberstring != null) && (customNumberstring != "")){
            String[] customEmergencyNumber = customNumberstring.split(";");
            for (int i = 0;i<customEmergencyNumber.length;i++){
                if(customEmergencyNumber[i].equals(number)||customEmergencyNumber[i].equals("+"+number)){
                    return true;
                }
                if(customEmergencyNumber[i].equals("-"+number)){
                    return false;
                }
            }
        }
