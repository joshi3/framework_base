...
///Support for ePDG
import com.android.server.connectivity.NetworkAgentInfo.HandoffType;
...
public class ConnectivityService extends IConnectivityManager.Stub
    ...
    /** M: for ePDG feature. {@ */
    private static final int EVENT_HANDOFF_EXPIRED = 100;
    /* @} */
    ...
    /** M: for ePDG feature. {@ */
    private static final int HANDOFF_TRANSFER_TIME = 60 * 1000;
    ...
    private static int sHandoffRadioType = ConnectivityManager.TYPE_NONE;
    ...
    private void assignNextNetId(NetworkAgentInfo nai) {
        ...
        if (sIsEpdgSupported) {
            ...
            if (oldNai != null && oldNai.network.netId != -1) {
                ...
                if (VDBG) {
                    log("Reuse handoff networkId:" + usedNetworkId);
                }
                notifyHandoverResult(true, true);
                nai.network = new Network(usedNetworkId);
                nai.handoffType = HandoffType.CONNECT;
                return;
            }
        }
...
//        mNetTrackers[info.getType()].captivePortalCheckCompleted(isCaptivePortal);
    ...
    private void handleAsyncChannelDisconnected(Message msg) {
        ...
        if (nai != null) {
            ...
            if (nai.created) {
                ...
                try {
                    ///M: Support for ePDG
                    if (nai.handoffType != HandoffType.DISCONNECT) {
    ...
    private void handleRegisterNetworkRequest(Message msg) {
        ...
        if (nri.isRequest) {
            ...
            ///M: Support for EPDG @{
            if (sIsEpdgSupported) {
                if (isHandoffSupport(nri.request.networkCapabilities)) {
    ...
    private class InternalHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                ///M: Support for ePDG
                case EVENT_HANDOFF_EXPIRED: {
                    handleHandoffExpired();
                    break;
                }
            }
        }
        ...
        if (state == NetworkInfo.State.CONNECTED && !networkAgent.created) {
            try {
                ...
                if (networkAgent.isVPN()) {
                    ...
                    if (networkAgent.handoffType == HandoffType.CONNECT) {
                        log("Handoff connection is connected:" + sHandoffRadioType);
                        mNetworkForNetId.put(networkAgent.network.netId, networkAgent);

                        //networkAgent.networkMisc.explicitlySelected = true;
                        if (sHandoffRadioType == ConnectivityManager.TYPE_MOBILE) {
                            ...
                            if (oldNai != null) {
                                ...
                                oldNai.handoffType = HandoffType.DISCONNECT;
                                log("get current score" + oldNai.getCurrentScore());
                            }
                        } else if (sHandoffRadioType == ConnectivityManager.TYPE_WIFI) {
                            NetworkAgentInfo oldNai = getExistedNetworkAgentInfo(networkAgent);
                            if (oldNai != null) {
                                oldNai.handoffType = HandoffType.DISCONNECT;
                            }
...
//        }
    ...
    public void connectToRadio(int radioType) {
        ...
        if (radioType != ConnectivityManager.TYPE_WIFI &&
            ...
            return;
        } else if (sHandoffRadioType != ConnectivityManager.TYPE_NONE) {
            log("Handover is ongoing:" + sHandoffRadioType);
            return;
        }
        ...
        if (isHandoff) {
            ...
            sHandoffRadioType = radioType;
            mHandler.sendMessageDelayed(mHandler.obtainMessage(
                        EVENT_HANDOFF_EXPIRED), HANDOFF_TRANSFER_TIME);
        }
    }
   ...
   /** M: for ePDG handover used. */
    private void retryRequestToNetwork(SparseArray<NetworkRequest> nrs, int newScore) {
        ...
        for (int i = 0; i < nrs.size(); i++) {
            ...
            if (isHandoffSupport(nr.networkCapabilities)) {
                for (NetworkFactoryInfo nfi : mNetworkFactoryInfos.values()) {
                    if (nfi.name.equals(nwTypeName)) {
                        nfi.asyncChannel.sendMessage(
                        android.net.NetworkFactory.CMD_REQUEST_NETWORK, newScore, 0, nr);
    ...
    private void handleEpdgDisconnect(NetworkAgentInfo nai) {
        ...
        if (keep && tryNewRadio == ConnectivityManager.TYPE_NONE) {
            ...
            log("No retry for ePDG/Mobile and notify app:" + nai.handoffType);
            if (nai.handoffType != HandoffType.DISCONNECT) {
                updateFailureCause(ratType, nai.networkCapabilities);
                notifyNetworkCallbacks(nai, ConnectivityManager.CALLBACK_LOST);
            } else {
                log("Handoff is running");
            }
        } else if (keep && !nai.created && nai.handoffType == HandoffType.NONE) {
            ...
            log("Retry policy:" + nai.handoffType + ":" + nai.created + ":" + keep);
            if (nai.created && nai.handoffType == HandoffType.DISCONNECT) {
               log("Disconnected by handoff");
            } else if(!nai.created && nai.handoffType == HandoffType.CONNECT) {
               log("Failed to setup new RAT on handover");
               sendHandoverIntent(sHandoffRadioType, false, false);
               sHandoffRadioType = ConnectivityManager.TYPE_NONE;
            }else {
               updateFailureCause(ratType, nai.networkCapabilities);
               notifyNetworkCallbacks(nai, ConnectivityManager.CALLBACK_LOST);
            }
    ...
    private int handleRnsSelection(int rnsRadioType) {
        ...
        if (rnsRadioType == RnsManager.ALLOWED_RADIO_WIFI) {
            ...
            sImsDisconnectCause = 110; //Deny by RNS policy.
        } else if (rnsRadioType == RnsManager.ALLOWED_RADIO_NONE) {
            score = NO_REQUEST_SCORE;
            sImsDisconnectCause = 111; //Deny by PS call, but call try CS call
        }
    ...
    private NetworkAgentInfo getExistedNetworkAgentInfo(NetworkAgentInfo nai) {
        ...
        if (nai.networkCapabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_IMS)) {
    ...
    private void handleHandoffExpired() {
        if (sHandoffRadioType != ConnectivityManager.TYPE_NONE) {
            Slog.d(TAG, "Handoff prcoedurer taks too long");
            sHandoffRadioType = ConnectivityManager.TYPE_NONE;
        }
    }

    private boolean isHandoffSupport(NetworkCapabilities cap) {
    ...
    private void notifyHandoverResult(boolean isOngoing, boolean isSucceed) {
        if (isOngoing) {
            sendHandoverIntent(sHandoffRadioType, true, true);
            return;
        }
        sendHandoverIntent(sHandoffRadioType, false, isSucceed);
        if (mHandler.hasMessages(EVENT_HANDOFF_EXPIRED)) {
            mHandler.removeMessages(EVENT_HANDOFF_EXPIRED);
        }
        sHandoffRadioType = ConnectivityManager.TYPE_NONE;
    }
    ...
    private void updateFailureCause(int networkType, NetworkCapabilities capability) {
        ...
        if (networkType == ConnectivityManager.TYPE_WIFI) {
            ...
            sImsDisconnectCause = epdgManager.getDisconnectCause(EpdgManager.TYPE_IMS);
            if (sImsDisconnectCause == 0) {
                NetworkInfo info = getNetworkInfo(networkType);
                if (info == null || !info.isConnected()) {
                    sImsDisconnectCause = 102; //Wi-Fi is disconnect and ePDG is disconnect
                }
            }
        } else {
            sImsDisconnectCause = 100; // Disconnect in LTE
        }
