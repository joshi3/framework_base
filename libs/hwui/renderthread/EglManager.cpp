...
void EglManager::requireGlContext(bool isFunctor) {
    ...
    if (!mInFrame) {
        ...
        if (!isFunctor || mCurrentSurface == EGL_NO_SURFACE) {
            usePBufferSurface();
        } else {
            /// M: [ALPS02025043] To avoid unnecessary eglMakeCurrent when invokeFunctor,
            /// fallback to check whether the current surface is valid by eglQuerySurface
            EGLint swapBehavior;
            EGLBoolean success = EGL_TRUE;
            TIME_LOG("eglQuerySurface",
                success = eglQuerySurface(mEglDisplay, mCurrentSurface, EGL_SWAP_BEHAVIOR, &swapBehavior));
            if (!success) {
                ALOGW("Failed to query EGL_SWAP_BEHAVIOR on surface %p, error=%p when invokeFunctor",
                                    (void*) mCurrentSurface, egl_error_str());
                usePBufferSurface();
            }
        }
    }
